...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        eval_result = (lambda operand: scheme_eval(operand, env))
        arguments = operands.map(eval_result)
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        new_operator = self.apply(operands, env)
        return scheme_eval(complete_eval(new_operator), env)
        # END Problem 21
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        rest_expression = scheme_eval(expressions.second.first, env)
        env.define(target, rest_expression)
        return target
        # END PROBLEM 6
...
def do_define_macro(expressions, env):
    ...
    args = expressions.first.second
    eval_body = expressions.second
    env.bindings[name] = MacroProcedure(args, eval_body, env)
    return name
    # END Problem 21
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            procedures = scheme_eval(first, env)
            check_procedure(procedures)
            result = procedures.eval_call(rest, env)
            # END PROBLEM 20
    return result
